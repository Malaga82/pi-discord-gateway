...
- `piscord setup` daemon install: the last `clack.spinner()` replaced with synchronous log lines — `runDaemon()` blocks the event loop with `execSync`/`spawnSync` (the spinner's first frame never drew) and its `stdio: 'inherit'` systemctl output would corrupt the redrawn line anyway.
